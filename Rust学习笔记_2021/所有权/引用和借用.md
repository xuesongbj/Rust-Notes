...
两者区别在于：<font color="yellow">引用必须有效</font>。引用失效前，目标必须存活且不可改变。

而指针未必，可能是空指针、野指针或悬垂指针。
...
* 可变引用(&mut T)失效(超出范围或不再使用)前，<font color="yellow">不能访问原变量</font>。
* 引用失效(超出范围或不再使用)前，<font color="yellow">原变量冻结(freeze)，不可修改，转移或重新赋值</font>。
