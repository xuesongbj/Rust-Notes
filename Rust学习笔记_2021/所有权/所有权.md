...
Copy和Clone特征传达了在代码中使用类型如何进行复制的原理。
...
没有实现Copy Trait的类型包括Vec<T>、String和可变引用。

#### Clone trait

Clone trait用于显式复制。Clone有一个clone方法，用于获取接收者的不可变引用，即`&self`，并返回相同类型的新值。`用户自定义类型或者需要提供能够复制自身的包装器类型，应通过实现clone方法实现Clone trait`。
